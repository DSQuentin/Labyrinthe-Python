...
class ListeJoueurs(object):
    ...
    def __add__(self, joueur):
        ...
        # comme on a une joueur de plus
        self.get_liste().append((pos_joueur, Joueur(joueur)))
    ...
    def distribuerTresors(self, nbTresors=24, nbTresorMax=0):
        ...
        cette fonction ne retourne rien mais modifie la liste des joueurs
        """
        nbTresors = int(nbTresors/len(self)) * \
            len(self)  # assure l'équité de la distribution
        # assure son côté aléatoire
        liste_distrib = sample(range(1, nbTresors+1), nbTresors)
