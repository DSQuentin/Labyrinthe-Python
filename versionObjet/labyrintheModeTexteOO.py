...
class LabyrintheModeTexte(object):
    ...
    def afficheLabyrinthe(self, message="", sauts=0):
        ...
        for i in range(1, labyrinthe.getNbParticipants()+1):
            pcouleur(liste_J.nomJoueur(i).ljust(10)+' ' +
                     str(liste_J.nbTresorsRestantsJoueur(i))+' ', i)
            print()
        print()
        ...
        for i in range(plateau.getNbLignes()):
            ...
            for j in range(plateau.getNbColonnes()):
                self.afficheCarte(plateau.getVal(
                    i, j), liste_J.numJoueurCourant(), tresor)
            if i % 2 == 0:
                print(' ', sep='', end='')
            else:
                print(str(i), sep='', end='')
    ...
    def saisirOrdre(self):
        ...
        résultat: l'ordre saisi sous la forme d'un couple (x,y)
                ...
                si l'ordre saisi n'est pas valide la fonction retourne (-1,-1)
        """
        valide = False
        while not valide:
            try:
                ...
                print(
                    "Au revoir et à bientôt dans ce très bon jeu, bientôt disponible en pré-vente")
    ...
    def saisirDeplacement(self):
        ...
        try:
            (x, y) = tuple(map(int, input(
                "Sous la forme num_lig num_col (5 4 par exemple), \n saisir votre déplacement : ").split()))
            if min(x, y) < 0 and max(x, y) > 6:
                (x, y) = (-1, -1)
        except KeyboardInterrupt:
            print("Merci d'être venu et à bientôt")
            raise KeyboardInterrupt
    ...
    def demarrer(self):
        ...
        while not fini:  # tant qu'aucun joueur n'a gagné
            while(labyrinthe.getPhase() == 1):  # tant qu'on est dans la phase 1
                ...
                # on effectue les actions liées à cet ordre
                res = labyrinthe.executerActionPhase1(x, y)
            ...
            while chemin == None:  # tant que la case destination n'est pas valide
                ...
                if xA == -1 or yA == -1:  # coordonées incorrectes
                    self.afficheLabyrinthe(
                        "Veuillez choisir une case du labyrinthe")
                    ...
                    if chemin == None:
                        self.afficheLabyrinthe(
                            "Cette case n'est pas accessible au joueur "+nomJC)
            ...
            if res2 == 2:
                ...
                message = "Le joueur "+nomJC + \
                    " vient de trouver le trésor "+str(t)
            self.afficheLabyrinthe(message)
