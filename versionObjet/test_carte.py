...
class TestCarte(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(16):
            self.liste_cartes.append(carte.Carte(
                i & self.N == self.N, i & self.E == self.E, i & self.S == self.S, i & self.O == self.O))
            self.liste_cartes_droite.append(carte.Carte(
                i & self.O == self.O, i & self.N == self.N, i & self.E == self.E, i & self.S == self.S))
    ...
    def test_murNord(self):
        for i in range(13):
            if i not in (7, 11):
                if i & self.N == self.N:
                    self.assertTrue(carte.murNord(self.liste_cartes[i]), "la carte "+str(
                        self.liste_cartes[i])+" devrait posséder un mur au nord")
                else:
                    self.assertFalse(carte.murNord(self.liste_cartes[i]), "la carte "+str(
    ...
    def test_murSud(self):
        for i in range(13):
            if i not in (7, 11):
                if i & self.S == self.S:
                    self.assertTrue(carte.murSud(self.liste_cartes[i]), "la carte "+str(
                        self.liste_cartes[i])+" devrait posséder un mur au sud")
                else:
                    self.assertFalse(carte.murSud(self.liste_cartes[i]), "la carte "+str(
                        self.liste_cartes[i])+" ne devrait pas posséder de mur au sud")

    def test_murOuest(self):
        for i in range(13):
            if i not in (7, 11):
                if i & self.O == self.O:
                    self.assertTrue(carte.murOuest(self.liste_cartes[i]), "la carte "+str(
                        self.liste_cartes[i])+" devrait posséder un mur à l'ouest")
                else:
                    self.assertFalse(carte.murOuest(self.liste_cartes[i]), "la carte "+str(
    ...
    def test_murEst(self):
        for i in range(13):
            if i not in (7, 11):
                if i & self.E == self.E:
                    self.assertTrue(carte.murEst(self.liste_cartes[i]), "la carte "+str(
                        self.liste_cartes[i])+" devrait posséder un mur à l'est")
                else:
                    self.assertFalse(carte.murEst(self.liste_cartes[i]), "la carte "+str(
