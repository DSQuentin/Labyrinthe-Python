...
class Plateau(object):
    def __init__(self, nbJoueurs, nbTresors):
        ...
        def creerCartesAmovibles(tresorDebut, nbTresors):
            ...
            """
            # creation plateau
            # on sait que les fixes on des coordonnées pairs
            liste_fixes = [9, 1, 1, 3, 8, 8, 1, 2, 8, 4, 12, 2, 12, 4, 4, 6]
            # on enlève les cartes fixes aux compteurs
            cpt_murs = {'D': [tout_droit, 12], 'J': [
                code_jonction, 18-12], 'A': [codeangle, 20-4]}

            laby = Matrice(7, 7, None)
            # sert pour la répartition des trésors
            list_coord = genere_Liste_coords(7, 7)
                ...
                for j in range(7):
                    if i % 2 or j % 2:  # si un seul des deux est impair
                        ...
                        if (i, j) in list_coord:
                            # un trésor sur chaque carte fixe sauf les coins
                            carte.mettreTresor(next(num_tresor, 0))
            ...
            for num_tresor in range(next(num_tresor, 50), nbTresors):
                ...
                # permet de mettre un seul tresor a chaque endroit
                list_coord.remove((i, j))
    ...
    def getCoordonneesCarte(self, carte):
        """
        retourne les coordonnées sous la forme (lig,col) de la carte passé en paramètre
        paramètres: plateau: le plateau considéré
                    numJoueur: le numéro du joueur à trouver
        resultat: un couple d'entier donnant les coordonnées de la carte ou None si
                la carte n'est pas sur le plateau
        """
        for i in range(7):
            for j in range(7):
                if self['P'][(i, j)] == carte:
        ...
        for e, (card, (i2, j2)) in enumerate(voisins):
            if not carte[card]:
                ...
                if 7 > i1 >= 0 and 7 > j1 >= 0:
                    carte2 = self['P'][(i1, j1)]
                    # cardinalité opposée à celle actuelle
                    card_opposee = voisins[3-e][0]
