...
class Labyrinthe(object):
    ...
    def check_new_Amovible(self, old_carte, new_carte, direction, rangee):
        """
        vérifie que le joueur reste sur le plateau s'il est sur la carte amovible
        aucun résultat, modifie juste
        """
        if old_carte.getNbPions():
            p = old_carte.getListePions().copy()
            (i, j) = self['P'].getCoordonneesCarte(new_carte)
            if direction == 'N' or direction == 'S':
                j = rangee
            else:
                i = rangee
            carte = self['P'][0][(i, j)]
            old_carte.setListePions([].copy())
            carte.setListePions(p)
    ...
    def jouerCarte(self, direction, rangee):
        ...
        """
        dir_rang = ('decalageColonneEnHaut', 'S'), ('decalageLigneADroite',
                                                    'O'), ('decalageLigneAGauche', 'E'), ('decalageColonneEnBas', 'N')
        mat = self.getPlateau()
        if direction == 'S':
            d = 'SN'  # représente la direction avec la direction opposée
        ...
        self.check_new_Amovible(Amovible, self.getCarteAJouer(), d[0], rangee)
        self['P'].amovible = Amovible
        self['C'] = (d[1], rangee)

    # c'est fort regrettable mais cette fonction souffre d'une erreur de pointeur
    def jouerCarte1(self, direction, rangee):
        ...
        dir_rang = ('decalageColonneEnHaut', 'S'), ('decalageLigneADroite',
                                                    'O'), ('decalageLigneAGauche', 'E'), ('decalageColonneEnBas', 'N')
        for e, (func, card) in enumerate(dir_rang):
            if card == direction:
                mat = self.getPlateau()

                commande = eval(
                    ""'mat.{0}(int(rangee),self["P"]["A"])'.format(func))
                # obligé de faire eval comme ce sont des méthodes les fonctions
                self['C'] = (dir_rang[3-e][1], rangee)
                self['P']['A'] = commande
    ...
    def finirTour(self):
        ...
        if posJ == posT:
            self['P'].prendreTresorPlateau(
                posT[0], posT[1], self.getTresorCourant())
