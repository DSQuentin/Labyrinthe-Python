...
class LabyrintheGraphique(object):
    ...
    def __init__(self, labyrinthe, titre='Labyrinthe des IUT', size=(1000, 800), couleur=(209, 238, 238), prefixeImage="./images"):
        ...
        self.laMatrice = labyrinthe.getPlateau()
    ...
    def surfaceCarte(self, carte):
        ...
        surfCarte = pygame.transform.smoothscale(
            img, (self.deltal, self.deltah))
        if t != 0:
        ...
        dist = 10
        coord = [(dist, dist), (dist, self.deltah-(self.deltah//4+dist)), (self.deltal-(self.deltal //
                                                                                        4+dist), self.deltah-(self.deltah//4+dist)), (self.deltal-(self.deltal//4+dist), dist)]
        for pion in p:
    ...
    def surfaceFleche(self, direction='O', couleur=(209, 238, 238)):
        ...
        pygame.draw.polygon(res, couleur, [(self.deltal//2, self.deltah//3), (self.deltal -
                                                                              self.deltal//8, self.deltah//2), (self.deltal//2, self.deltah*2//3)], 0)
        if direction == 'N':
    ...
    def afficheScore(self, numLigne=3):
        ...
        """
        listeJoueurs = self.labyrinthe.getListeJoueurs()
        texte = "Nb trésors restants:"
        img = []
        for i in range(listeJoueurs.getNbJoueurs()):
            texte += " @img@ "+str(listeJoueurs.nbTresorsRestantsJoueur(i+1))
    ...
    def afficheJoueurs(self, couleur=None):
        ...
        for pion in range(1, listeJoueurs.getNbJoueurs()+1):
            nom = listeJoueurs.nomJoueur(pion)
    ...
    def afficheCarteAJouer(self):
        ...
        """
        self.surface.blit(self.surfaceCarte(
            self.labyrinthe.getCarteAJouer()), (self.finl+self.deltal//2, self.finh//2))
    ...
    def dessineGrille(self, couleur=(255, 0, 0)):
        ...
        for i in range(1, self.nbCol, 2):
            ...
            self.surface.blit(flecheS, ((i+1)*self.deltah,
                                        self.deltah*(self.nbLig+1)))
        ...
        for i in range(self.nbLig):
            for j in range(self.nbCol):
                try:
                    ...
                    if s == None:
                        self.surface.fill(
                            (0, 0, 0), ((j+1)*self.deltal, (i+1)*self.deltah, self.deltal, self.deltah))
                    else:
                        self.surface.blit(
    ...
    def afficheJeu(self):
        ...
        self.afficheGrille()
        if not self.fini:
            self.afficheMessage(2, "C'est au joueur @img@ de jouer. Trésor à trouver @img@", [self.surfacePion(
                self.labyrinthe.getListeJoueurs().numJoueurCourant()), self.surfaceTresor(self.labyrinthe.getListeJoueurs().tresorCourant())])
        self.afficheScore(3)
    ...
    def demarrer(self):
        ...
        while(True):
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.labyrinthe.getPhase() == 1:
                    res = self.labyrinthe.executerActionPhase1(x, y)
                    if res == 0:
                    ...
                    if x in ['N', 'E', 'S', 'O', 'T'] or x == -1 or y == -1:
                        ...
                        chemin = self.labyrinthe.accessibleDistJoueurCourant(
                            x, y)
                        jc = self.labyrinthe.getListeJoueurs().numJoueurCourant()
                        if chemin == None:
                            ...
                            t = self.labyrinthe.getListeJoueurs().tresorCourant()
                            res2 = self.labyrinthe.finirTour()
                            if res2 == 2:
                                ...
                                self.imgInfo = [self.surfacePion(
                                    jc), self.surfaceTresor(t)]
...
if __name__ == "__main__":
    ...
    while continuer:
        liste_J.append(
            input("Joueur numéro {0},\nVeuillez insérer son nom  : ".format(i)))
        i += 1
        if i == 5:
            continuer = False
        rep = input(
            "Un autre joueur veut-il se joindre à cette partie ma foi déjantée ? (o/N)")
        if rep == 'o' or rep == 'O':
            continue
        continuer = False
    ...
    while not ok:
        try:
            nbTresors = int(input(
                "Combien de trésors est-ce que les joueurs devront trouver (0 pour le maximum) ? "))
            ok = True
        except:
