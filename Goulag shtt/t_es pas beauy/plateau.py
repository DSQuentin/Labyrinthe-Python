...
class Plateau(object):
    def __init__(self,nbJoueurs,nbTresors):
        ...
        for (i,j),joueur in zip(((0, 0), (6, 6), (0, 6),(6,0)),sample(range(nbJoueurs),nbJoueurs)):
            carte = getVal(self.laby,i,j)
            poserPion(carte,joueur)
            setVal(self.laby,i,j,carte)
    ...
    def __iter__(self):
        """
        renvoie toutes les cartes du plateau avec leur coordonnées ainsi que la carte amovible et ses coordonnées "(None,None)"
        """
        for i in range(7):
            for j in range(7):
                yield ((i,j),getVal(self.laby,i,j))
        yield ((None,None),self.amovible)
...
def getCoordonneesTresor(plateau,numTresor):
    ...
    for i in range(7):
        for j in range(7):
            carte = getVal(plateau.laby,i,j)
            print(getTresor(carte),(i,j))
