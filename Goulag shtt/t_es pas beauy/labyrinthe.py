...
class Cl_labyrinthe(object):
    def __init__(self,nomsJoueurs,nbTresors,nbTresorsMax):
        self.joueurs = ListeJoueurs(nomsJoueurs)
        self._plateau = Plateau(len(self.joueurs),nbTresors)
        distribuerTresors(self.joueurs,nbTresors,nbTresorsMax)
        self._gene = (cycle(p for p in range(1,3)),cycle(player for player in self.joueurs))
        self._cur_phase = next(self._gene[0])
        self._cur_player = next(self._gene[1])
    ...
    def __next__(self):
        ...
        """
        self._cur_phase = next(self._gene[0])
        if self._cur_phase == 1:
            self._cur_player = next(self._gene[1]) 

    def __iter__(self):
        """
        renvoie tous les joueurs contenus dans le labyrinthe
        """
...
def Labyrinthe(nomsJoueurs=["joueur1","joueurs2"],nbTresors=24, nbTresorsMax=0):
    ...
    """
    return Cl_labyrinthe(nomsJoueurs,nbTresors,nbTresorsMax)
...
def getNbParticipants(labyrinthe):
    ...
    """
    return labyrinthe['P'].get_nb_joueurs()
...
def getNomJoueurCourant(labyrinthe):
    ...
    """
    return labyrinthe.current_player()
...
def getNbTresors(labyrinthe):
    ...
    """
    somme = 0
    for _, joueur in labyrinthe:
        somme += len(joueur.get_tresors())
    return somme
...
def enleverTresor(labyrinthe,lin,col,numTresor):
    ...
    la fonction ne retourne rien mais modifie le labyrinthe
    """
    for perso in labyrinthe.joueur :
        for i,tresor in enumerate(perso):
            if tresor == numTresor:
                perso - i
...
def coupInterdit(labyrinthe,direction,rangee):
    ...
    """
    direction.replace('O','W')
    return not labyrinthe['P'][direction]   
...
def jouerCarte(labyrinthe,direction,rangee):
    ...
    """
    direction.replace('O','W')
    dir_rang = (decalageLigneAGauche,'W'),(decalageLigneADroite,'E'),(decalageColonneEnHaut,'N'),(decalageColonneEnBas,'S')
    for func,card in dir_rang:
        if card == direction:
            func(labyrinthe['P'].laby,card,rangee)
...
def tournerCarte(labyrinthe,sens='H'):
    ...
    """
    if sens == 'H':
        tournerHoraire(labyrinthe['P'])
    else:
        tournerAntiHoraire(labyrinthe['P'])
...
def getCoordonneesJoueurCourant(labyrinthe):
    ...
    for (i,j),card in labyrinthe['P']:
        if num in card: #check si c'est num ou son nom 
            return (i,j)
...
def executerActionPhase1(labyrinthe,action,rangee):
    ...
    """
    if action == 'T':
        tournerCarte(labyrinthe['P'].amovible)
    else:
...
def accessibleDistJoueurCourant(labyrinthe, ligA,colA):
    ...
    """
    i,j = getCoordonneesJoueurCourant(labyrinthe)
    return accessibleDist(getPlateau(labyrinthe),i,j,ligA,colA)
...
def finirTour(labyrinthe):
    ...
    if posJ == posT:
        prendreTresorPlateau(getPlateau(labyrinthe),i,j)
