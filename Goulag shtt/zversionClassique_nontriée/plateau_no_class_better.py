...
def code_jonction():
    ...
    liste = [1,2,4,8]
    shuffle(liste) #on positionne le False aléatoirement
    ...
    for i in range(7):
        for j in range(7):
            if i%2 or j%2: # si un seul des deux est impair
                ...
                liste_fixes.pop(0)
                mettreTresor(carte,next(num_tresor,0)) #un trésor sur chaque carte fixe
    ...
    if (ligD,colD) == (ligA,colA):
        ...
        for i1,j1 in check_voisins(carte,colD,ligD):
            if (i1,j1) not in visited:
                return accessible(plateau,i1,j1,ligA,colA, visited + [(i1,j1)])
