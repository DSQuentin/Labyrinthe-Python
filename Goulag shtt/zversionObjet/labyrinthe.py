...
class Cl_labyrinthe(object):
    def __init__(self,nomsJoueurs,nbTresors,nbTresorsMax):
        self._joueurs = ListeJoueurs(nomsJoueurs)
        self._plateau = Plateau(len(self._joueurs),nbTresors)
        distribuerTresors(self._joueurs,nbTresors,nbTresorsMax)
        self._gene = zip(cycle(p for p in range(1,3)),cycle(player for player in sum(zip(self._joueurs, self._joueurs), tuple())))
        self._cur_phase,self._cur_player = next(self._phase)
    ...
    def __next__(self):
        ...
        """
        self._cur_phase,self._cur_player = next(self._gene)
...
def getNumJoueurCourant(labyrinthe):
    ...
    """
    return getJoueurCourant(labyrinthe['J'])
...
def poserJoueurCourant(labyrinthe,lin,col):
    ...
    la fonction ne retourne rien mais modifie le labyrinthe     
    """
    joueur = prendreJoueurCourant(laby)
...
def getTresorCourant(labyrinthe):
    ...
    """
    return getJoueurCourant(labyrinthe).get_tresor_i()
