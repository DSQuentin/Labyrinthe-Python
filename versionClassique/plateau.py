...
def creerCartesAmovibles(tresorDebut, nbTresors):
    ...
    cpt_murs = {'D': [tout_droit, 12], 'J': [code_jonction, 18-12],
                'A': [codeangle, 20-4]}  # on enlève les cartes fixes aux compteurs
        ...
        for j in range(7):
            if i % 2 or j % 2:  # si un seul des deux est impair
                ...
                if (i, j) in list_coord:
                    ...
                    mettreTresor(carte, next(num_tresor, 0))
...
def getCoordonneesCarte(plateau, carte):
    """
    ...
    for i in range(7):
        for j in range(7):
            if getVal(getMatrice(plateau), i, j) == carte:
                return (i, j)
