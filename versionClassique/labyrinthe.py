...
def getNbTresors(labyrinthe):
    ...
    """
    somme = 0
    for num in range(getNbParticipants(labyrinthe)):
...
def check_new_Amovible(labyrinthe, old_carte, new_carte, direction, rangee):
    """
    vérifie que le joueur reste sur le plateau s'il est sur la carte amovible
    aucun résultat, modifie juste
    """
    if getNbPions(old_carte):
        p = getListePions(old_carte).copy()
        (i, j) = getCoordonneesCarte(labyrinthe['P'], new_carte)
        # if direction =='N' or direction =='S':
        #     j = rangee
        # else:
        #     i = rangee
        carte = getVal(getPlateau(labyrinthe), i, j)
        setListePions(old_carte, [].copy())
        setListePions(carte, p)
...
def jouerCarte(labyrinthe, direction, rangee):
    ...
    for e, (func, card) in enumerate(dir_rang):
        if card == direction:
            Amovible = func(getPlateau(labyrinthe), int(
                rangee), getAmovible(labyrinthe['P']))
            check_new_Amovible(labyrinthe, Amovible, getCarteAJouer(
                labyrinthe), direction, rangee)
            labyrinthe['P'][1] = Amovible
            labyrinthe['C'] = (dir_rang[3-e][1], rangee)
...
def finirTour(labyrinthe):
    ...
    if posJ == posT:
        prendreTresorPlateau(
            labyrinthe['P'], posT[0], posT[1], getTresorCourant(labyrinthe))
