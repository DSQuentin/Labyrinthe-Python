...
def afficheLabyrinthe(lmt, message="", sauts=0):
    ...
    for i in range(1, getNbParticipants(labyrinthe)+1):
        pcouleur(nomJoueur(listeJoueurs, i).ljust(10)+' ' +
                 str(nbTresorsRestantsJoueur(listeJoueurs, i))+' ', i)
        print()
    print()
    ...
    for i in range(getNbLignes(plateau)):
        ...
        for j in range(getNbColonnes(plateau)):
            afficheCarte(lmt, getVal(plateau, i, j),
                         numJoueurCourant(listeJoueurs), tresor)
        if i % 2 == 0:
            print(' ', sep='', end='')
        else:
            print(str(i), sep='', end='')
    ...
    while not fini:  # tant qu'aucun joueur n'a gagné
        ...
        while chemin == None:  # tant que la case destination n'est pas valide
            ...
            if xA == -1 or yA == -1:  # coordonées incorrectes
                afficheLabyrinthe(
                    lmt, "Veuillez choisir une case du labyrinthe")
                ...
                if chemin == None:
                    afficheLabyrinthe(
                        lmt, "Cette case n'est pas accessible au joueur "+nomJC)
